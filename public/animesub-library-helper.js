(() => {
  ...
  function enhanceAnimeSubButtons() {
    ...
    if (episodeCards.length) { installAnimeSubEpisodeButtons(detail, mediaId, episodeCards); summarizeSeriesSubtitleStatus(detail, episodeCards); return; }
  ...
  async function summarizeSeriesSubtitleStatus(detail, episodeCards) {
    if (detail.dataset.animesubSummaryLoading === 'true') return;
    detail.dataset.animesubSummaryLoading = 'true';
    try {
      const ids = episodeCards.map((card) => card.getAttribute('data-animesub-episode-id')).filter(Boolean);
      ...
      showSeriesSummary(detail, done, uniqueIds.length, totalSubtitles);
    } finally {
      detail.dataset.animesubSummaryLoading = 'false';
    }
  }

  function showSeriesSummary(detail, done, total, totalSubtitles) {
    const grid = detail.querySelector('.library-detail-grid');
    if (!grid) return;
    let box = grid.querySelector('[data-animesub-series-summary]');
    if (!box) {
      box = document.createElement('div');
      box.className = 'library-detail-box';
      box.dataset.animesubSeriesSummary = 'true';
      grid.appendChild(box);
    }
    box.innerHTML = `<span>Napisy AnimeSub</span><strong>${done}/${total} odcinków</strong><small>${totalSubtitles} plików napisów w cache</small>`;
  }
  ...
  async function checkCachedSubtitleStatus(type, id, card) {
    ...
    if (cached && cached.count !== undefined) { if (cached.count > 0) showAnimeSubBadge(card, cached.count, false, true); else showAnimeSubBadge(card, 0, false, true); return; }
    cacheStatus.set(key, 'loading');
    try {
      const count = await getSubtitleCount(type, id);
      showAnimeSubBadge(card, count, false, true);
    } catch { cacheStatus.delete(key); }
  }
  ...
  async function forceAnimeSub(button, type, id, card) {
    ...
    try {
      ...
      showAnimeSubBadge(card, count, false, true);
      const detail = document.querySelector('#libraryMediaDetails:not(.hidden)');
      if (detail) summarizeSeriesSubtitleStatus(detail, Array.from(detail.querySelectorAll('.library-episode-card')));
  ...
  function showAnimeSubBadge(card, count, error = false, cached = false) {
    ...
    if (!card && target.classList.contains('library-detail-grid')) {
      let box = target.querySelector('[data-animesub-movie-summary]');
      if (!box) { box = document.createElement('div'); box.className = 'library-detail-box'; box.dataset.animesubMovieSummary = 'true'; target.appendChild(box); }
      box.innerHTML = `<span>Napisy AnimeSub</span><strong>${count > 0 ? `${count} plików` : 'brak w cache'}</strong>`;
      return;
    }
