(() => {
  ...
  function showAnimeSubPopup(report) {
    ...
    backdrop.querySelectorAll('[data-preview-subtitle-url]').forEach((button) => {
      button.addEventListener('click', () => loadSubtitlePreview(button, backdrop));
    });
    document.body.appendChild(backdrop);
  }

  function renderSubtitleTable(subtitles) {
    return `<div class="animesub-table-wrap"><table class="animesub-table"><thead><tr><th>Język</th><th>Nazwa</th><th>Format</th><th>Addon</th><th>URL</th><th>Akcje</th></tr></thead><tbody>${subtitles.map((subtitle) => {
      const format = inferSubtitleFormat(subtitle.url, subtitle.name);
      return `<tr><td>${escapeHtml(subtitle.lang || '-')}</td><td>${escapeHtml(subtitle.name || subtitle.id || '-')}</td><td>${escapeHtml(format)}</td><td>${escapeHtml(subtitle.addonName || subtitle.addonId || '-')}</td><td>${subtitle.url ? `<a href="${escapeHtml(subtitle.url)}" target="_blank" rel="noreferrer">Otwórz</a>` : '-'}</td><td>${subtitle.url ? `<button class="animesub-mini-btn" type="button" data-preview-subtitle-url="${escapeHtml(subtitle.url)}">Podgląd pliku</button>` : '-'}</td></tr>`;
  ...
  async function loadSubtitlePreview(button, root) {
    ...
    try {
      const response = await fetch(`/admin/animesub/subtitle-preview?url=${encodeURIComponent(url)}`);
      const data = await response.json().catch(() => ({}));
      if (!response.ok || data.error) throw new Error(data.error || `HTTP ${response.status}`);
      const header = [
        `Format: ${data.format || inferSubtitleFormat(url)}`,
        `Content-Type: ${data.contentType || '-'}`,
        `Rozmiar: ${formatBytes(data.contentLength || data.bytesRead)}`,
        `Pobrano: ${formatBytes(data.bytesRead)}`,
        data.truncated ? 'Uwaga: podgląd skrócony do pierwszych 256 KB.' : ''
      ].filter(Boolean).join('\n');
  ...
  function inferSubtitleFormat(url = '', name = '') {
    ...
    if (/\.srt\b/.test(value) || /subrip/.test(value)) return 'srt';
