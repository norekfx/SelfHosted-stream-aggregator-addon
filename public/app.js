...
function bindNavigation() {
  ...
  $('#forwardedSubtitleMode')?.addEventListener('change', updateForwardedSubtitleLanguageVisibility);
}
...
function bindForms() {
  ...
  $('#settingsForm').addEventListener('submit', async (event) => {
    event.preventDefault();
    const body = { preferredAudioLanguage: $('#preferredAudioLanguage').value || 'pl', preferredSubtitleLanguage: $('#preferredSubtitleLanguage').value || 'pl', forwardExternalSubtitles: $('#forwardExternalSubtitles').checked, forwardedSubtitleMode: $('#forwardedSubtitleMode').value || 'language', forwardedSubtitleLanguage: $('#forwardedSubtitleLanguage').value || 'pl', defaultTranscodeBufferPreset: $('#defaultTranscodeBufferPreset').value, streamValidationTimeoutMs: Number($('#streamValidationTimeoutMs').value), maxTranscodeSessions: Number($('#maxTranscodeSessions').value), publicBaseUrl: $('#publicBaseUrl').value.trim(), autoRefreshCache: $('#autoRefreshCache').checked, showDiagnosticDetails: $('#showDiagnosticDetails').checked };
...
function renderSettingsSummary() { const s = state.settings ?? {}; const forwarded = s.forwardExternalSubtitles === false ? 'wyłączone' : (s.forwardedSubtitleMode === 'all' ? 'wszystkie' : `tylko ${languageLabel(s.forwardedSubtitleLanguage ?? 'pl')}`); $('#settingsSummary').innerHTML = [['Audio', languageLabel(s.preferredAudioLanguage)], ['Napisy preferowane', languageLabel(s.preferredSubtitleLanguage)], ['Przekazywanie napisów', forwarded], ['Bufor', s.defaultTranscodeBufferPreset], ['Timeout walidacji', `${s.streamValidationTimeoutMs ?? '-'} ms`], ['Sesje transkodowania', s.maxTranscodeSessions], ['Publiczny URL', s.publicBaseUrl || 'nie ustawiono']].map(([key, value]) => `<div class="kv"><span>${key}</span><strong>${escapeHtml(String(value ?? '-'))}</strong></div>`).join(''); }
function renderInstall() { const manifestUrl = getManifestUrl(); const testStreamUrl = getTestStreamUrl(); $('#manifestInstallUrl').textContent = manifestUrl; $('#testStreamUrl').textContent = testStreamUrl; const baseUrl = getInstallBaseUrl(); const healthStatus = state.healthReport?.status ?? 'warn'; const hasHttps = baseUrl.startsWith('https://'); const hasAddons = state.addons.length > 0; const hasOnlineAddon = state.addons.some((addon) => addon.status === 'online'); const hasSubtitleAddon = state.addons.some((addon) => addon.enabled && addon.status === 'online' && (addon.supportedResources ?? []).includes('subtitles') && !addon.detectedAnimeSub); const hasAnimeSub = state.addons.some((addon) => addon.detectedAnimeSub && addon.enabled && addon.status === 'online'); const checks = [['Publiczny URL', hasHttps ? 'ok' : 'warn', hasHttps ? 'Skonfigurowano HTTPS.' : 'Dla zdalnego Stremio/Nuvio ustaw HTTPS w Publicznym URL.'], ['Manifest', 'ok', 'Endpoint manifestu pozostaje publiczny.'], ['Playback bez logowania', 'ok', 'Stream/proxy/transcode endpointy są publiczne dla klientów.'], ['Napisy z addonów', hasSubtitleAddon ? 'ok' : 'warn', hasSubtitleAddon ? 'Wykryto aktywny subtitle-addon do przekazywania napisów.' : 'Dodaj aktywny addon z zasobem subtitles, aby przekazywać napisy spoza AnimeSub.'], ['Napisy AnimeSub', hasAnimeSub ? 'ok' : 'warn', hasAnimeSub ? 'Wykryto aktywny addon AnimeSub.' : 'Dodaj animesub.info, aby ręcznie pobierać napisy dla anime.'], ['Addony', hasAddons ? (hasOnlineAddon ? 'ok' : 'warn') : 'warn', hasAddons ? (hasOnlineAddon ? 'Masz co najmniej jeden addon online.' : 'Addony są dodane, ale żaden nie jest online.') : 'Dodaj co najmniej jeden zewnętrzny addon.'], ['Health-check', healthStatus, state.healthReport ? `Ostatni status: ${healthStatus}.` : 'Uruchom test w zakładce System.']]; $('#installChecklist').innerHTML = checks.map(([label, status, message]) => `<div class="check-item"><div><strong>${escapeHtml(label)}</strong><br><span>${escapeHtml(message)}</span></div>${badge(status)}</div>`).join(''); }
...
function renderLanguageSelects() { const options = getLanguageOptions().map((language) => `<option value="${escapeHtml(language.code)}">${escapeHtml(language.label ?? `${language.nativeName ?? language.code} / ${language.englishName ?? language.code}`)}</option>`).join(''); const audio = $('#preferredAudioLanguage'); const subtitles = $('#preferredSubtitleLanguage'); const forwarded = $('#forwardedSubtitleLanguage'); if (audio) audio.innerHTML = options; if (subtitles) subtitles.innerHTML = options; if (forwarded) forwarded.innerHTML = options; }
function renderSettings() { const s = state.settings ?? {}; renderLanguageSelects(); $('#preferredAudioLanguage').value = s.preferredAudioLanguage ?? 'pl'; $('#preferredSubtitleLanguage').value = s.preferredSubtitleLanguage ?? 'pl'; $('#forwardExternalSubtitles').checked = s.forwardExternalSubtitles !== false; $('#forwardedSubtitleMode').value = s.forwardedSubtitleMode ?? 'language'; $('#forwardedSubtitleLanguage').value = s.forwardedSubtitleLanguage ?? 'pl'; $('#defaultTranscodeBufferPreset').value = s.defaultTranscodeBufferPreset ?? 'auto'; $('#streamValidationTimeoutMs').value = s.streamValidationTimeoutMs ?? 10000; $('#maxTranscodeSessions').value = s.maxTranscodeSessions ?? 2; $('#publicBaseUrl').value = s.publicBaseUrl ?? ''; $('#autoRefreshCache').checked = s.autoRefreshCache !== false; $('#showDiagnosticDetails').checked = s.showDiagnosticDetails !== false; updateForwardedSubtitleLanguageVisibility(); }
function updateForwardedSubtitleLanguageVisibility() { const row = $('#forwardedSubtitleLanguageRow'); const mode = $('#forwardedSubtitleMode')?.value ?? state.settings?.forwardedSubtitleMode ?? 'language'; if (row) row.classList.toggle('hidden', mode !== 'language'); }
