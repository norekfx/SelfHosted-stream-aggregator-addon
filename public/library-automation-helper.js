(() => {
  ...
  function installAutomationFields() {
    ...
    if (!grid || document.querySelector(`#${DOCCHI_FIELD_ID}`)) return;
    ...
    document.querySelector(`#${DOCCHI_FIELD_ID}`)?.addEventListener('change', syncIntervalVisibility);
  ...
  function intervalOptions() {
    return `<option value="24">24 h</option><option value="72">3 dni</option><option value="168" selected>7 dni</option><option value="336">14 dni</option><option value="720">30 dni</option>`;
  }

  function syncIntervalVisibility() {
    const docchi = document.querySelector(`#${DOCCHI_FIELD_ID}`)?.value || 'inherit';
    const animeSub = document.querySelector(`#${ANIMESUB_FIELD_ID}`)?.value || '7d';
    const docchiWrap = document.querySelector('[data-docchi-interval-wrap]');
    const animeWrap = document.querySelector('[data-animesub-interval-wrap]');
    if (docchiWrap) docchiWrap.style.display = docchi === 'disabled' ? 'none' : '';
    if (animeWrap) animeWrap.style.display = animeSub === 'manual' ? 'none' : '';
  }
  ...
  function readAutomationConfig() {
    const docchiMode = document.querySelector(`#${DOCCHI_FIELD_ID}`)?.value || 'inherit';
    const animeSubMode = document.querySelector(`#${ANIMESUB_FIELD_ID}`)?.value || '7d';
    const config = {
      docchiAutomationMode: docchiMode,
      animeSubAutomationMode: animeSubMode
    };
  ...
  function installDashboardProgressBox() {
    ...
    panel.innerHTML = '<div class="panel-header"><h2>Automatyzacja bibliotek</h2></div><div id="libraryAutomationProgress" class="list empty">Brak aktywnego indeksowania.</div>';
  ...
  async function refreshAutomationProgress() {
    ...
    try {
      ...
      if (!active.length) { box.innerHTML = 'Brak aktywnego indeksowania.'; return; }
      box.innerHTML = active.map((item) => {
        const pct = item.total ? Math.round((item.done / item.total) * 100) : 0;
        return `<div class="kv"><span>${escapeHtml(item.libraryId)} · ${escapeHtml(item.task)}</span><strong>${escapeHtml(item.done)} / ${escapeHtml(item.total)} (${pct}%)</strong></div>`;
      }).join('');
    } catch {
