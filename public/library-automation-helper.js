(() => {
  ...
  function installAutomationFields() {
    ...
    if (enabled?.parentNode === grid) grid.insertBefore(docchiMode, enabled), grid.insertBefore(docchiInterval, enabled), grid.insertBefore(animeSubMode, enabled), grid.insertBefore(animeSubInterval, enabled);
    else grid.append(docchiMode, docchiInterval, animeSubMode, animeSubInterval);
  ...
  async function refreshAutomationProgress() {
    ...
    try {
      ...
      if (!active.length) { box.innerHTML = `<div class="kv"><span>Status</span><strong>${control.paused ? 'Wstrzymana' : control.running ? 'Pracuje' : 'Bezczynna'}</strong></div><p class="hint">Przyciski powyżej sterują kolejnym lub aktualnym przebiegiem automatyzacji.</p>`; return; }
      box.innerHTML = active.map((item) => { const pct = item.total ? Math.round((item.done / item.total) * 100) : 0; const status = item.status === 'paused' ? 'wstrzymane' : 'trwa'; return `<div class="kv"><span>${escapeHtml(item.libraryId)} · ${escapeHtml(item.task)} · ${status}</span><strong>${escapeHtml(item.done)} / ${escapeHtml(item.total)} (${pct}%)</strong></div>`; }).join('');
    } catch { box.innerHTML = '<p class="hint">Nie udało się pobrać statusu automatyzacji. Sprawdź, czy backend ma endpoint /admin/library-automation/status.</p>'; }
  }
