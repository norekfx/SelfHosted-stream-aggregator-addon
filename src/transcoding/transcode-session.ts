...
import { enforceTranscodeCacheLimit, writeTranscodeCacheInfo } from "./transcode-cache-manager.js";

export type TranscodeSessionStatus = "starting" | "running" | "exited" | "failed";

export type TranscodeSpeedStats = { samples: number; average?: number; min?: number; max?: number };

export type TranscodeSession = {
  id: string; streamId: string; title?: string; sourceAddon?: string; sourceQuality?: string; quality: TranscodeQuality; bufferPreset: BufferPreset; originalUrl: string; profile: TranscodeProfile; outputDir: string; playlistPath: string; status: TranscodeSessionStatus; startedAt: string; updatedAt: string; error?: string; lastLog?: string; stopReason?: string; progress?: { frame?: number; fps?: number; bitrate?: string; outTime?: string; speed?: string; progress?: string }; speedStats?: TranscodeSpeedStats; buffer?: { segmentCount: number; estimatedSeconds: number; segmentSeconds: number }; process?: ChildProcessWithoutNullStreams; qsv?: { requestedMode: string; runtimeMode: string; active: boolean; fallbackToCpu: boolean; reason?: string; fallbackReason?: string };
};
...
export function getOrCreateTranscodeSession(original: AggregatedStream, quality: TranscodeQuality, bufferPreset: BufferPreset): TranscodeSession {
  ...
  writeTranscodeCacheInfo(outputDir, { id: sessionId, mode: "live", title: session.title, streamId: original.id, quality, sourceQuality: original.quality, createdAt: session.startedAt });
...
function stopSession(session: TranscodeSession, reason: string, force = false): Omit<TranscodeSession, "process"> { session.stopReason = reason; session.error = undefined; session.status = "exited"; session.updatedAt = new Date().toISOString(); const child = session.process; if (child && !child.killed) { child.kill("SIGTERM"); if (force) setTimeout(() => { if (session.process && !session.process.killed) session.process.kill("SIGKILL"); }, 2_000).unref(); } updateBufferInfo(session); const { process: _process, ...snapshot } = session; writeSystemLog("info", "transcode", force ? "Live transcode session force stop requested." : "Transcode session stop requested.", { sessionId: session.id, streamId: session.streamId, quality: session.quality, reason }); return snapshot; }
...
function startFfmpegAttempt(session: TranscodeSession, qsvPlan: IntelQsvPlan, forcedCpu: boolean): void {
  ...
  child.on("exit", (code, signal) => { session.process = undefined; updateBufferInfo(session); session.updatedAt = new Date().toISOString(); enforceTranscodeCacheLimit(); if (session.stopReason) { session.status = "exited"; session.lastLog = session.stopReason; writeSystemLog("info", "transcode", "FFmpeg session stopped intentionally.", { sessionId: session.id, streamId: session.streamId, quality: session.quality, bufferPreset: session.bufferPreset, reason: session.stopReason, signal }); return; } if (code !== 0 && !forcedCpu && qsvPlan.enabled) { session.qsv = { ...(session.qsv ?? qsvSnapshot(qsvPlan)), active: false, runtimeMode: "cpu", fallbackToCpu: true, fallbackReason: `QSV FFmpeg exited with code ${code}${signal ? `, signal ${signal}` : ""}; retrying CPU libx264.` }; session.error = undefined; writeSystemLog("warn", "transcode", "Intel QSV failed for Live HLS; retrying with CPU libx264.", { sessionId: session.id, streamId: session.streamId, quality: session.quality, qsv: session.qsv, lastLog: session.lastLog }); startFfmpegAttempt(session, qsvPlan, true); return; } session.status = code === 0 ? "exited" : "failed"; if (code !== 0) { session.error = session.error ?? `FFmpeg exited with code ${code}${signal ? `, signal ${signal}` : ""}.`; writeSystemLog("error", "transcode", session.error, { sessionId: session.id, streamId: session.streamId, quality: session.quality, bufferPreset: session.bufferPreset, signal, qsv: session.qsv }); } });
}
