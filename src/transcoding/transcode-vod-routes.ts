...
type VodSession = { id: string; streamId: string; quality: TranscodeQuality; bufferPreset: BufferPreset; originalUrl: string; durationSeconds: number; segmentSeconds: number; outputDir: string; playlistPath: string; createdAt: string };
...
async function getOrCreateVodSession(streamId: string, quality: TranscodeQuality, bufferPreset: BufferPreset, originalUrl: string): Promise<VodSession> {
  ...
  const session: VodSession = { id: sessionId, streamId, quality, bufferPreset, originalUrl, durationSeconds, segmentSeconds: profile.hlsSegmentSeconds, outputDir, playlistPath: join(outputDir, "master.m3u8"), createdAt: new Date().toISOString() };
...
async function runVodSegmentFfmpeg(session: VodSession, segmentName: string): Promise<void> {
  ...
  const args = ["-hide_banner", "-loglevel", "warning", "-ss", String(startSeconds), "-reconnect", "1", "-reconnect_streamed", "1", "-reconnect_delay_max", "5", "-i", session.originalUrl, "-t", String(durationSeconds), "-map", "0:v:0", "-map", "0:a:0?", "-max_muxing_queue_size", "2048", "-vf", buildVideoFilter(profile.width, profile.height), "-c:v", "libx264", "-preset", profile.preset, "-crf", String(profile.crf), "-pix_fmt", "yuv420p", "-profile:v", "high", "-g", "48", "-keyint_min", "48", "-sc_threshold", "0"];
...
function runProcess(command: string, args: string[]): Promise<void> { return new Promise((resolve, reject) => { const child = spawn(command, args, { stdio: "pipe" }); let stderr = ""; child.stderr.on("data", (chunk) => { stderr += chunk.toString(); stderr = stderr.slice(-4000); }); child.on("error", reject); child.on("exit", (code, signal) => { if (code === 0) resolve(); else reject(new Error(stderr || `Process exited with code ${code}${signal ? `, signal ${signal}` : ""}.`)); }); }); }
