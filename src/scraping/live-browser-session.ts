...
export function getLiveBrowserState(id: string) { const session = requireSession(id); touch(session); return publicState(session); }
export async function getLiveBrowserScreenshot(id: string): Promise<Buffer> {
  const session = requireSession(id); touch(session);
  return Buffer.from(await session.page.screenshot({ type: "jpeg", quality: 68, fullPage: false, captureBeyondViewport: false }));
}

export async function clickLiveBrowser(id: string, xInput: number, yInput: number) {
  const session = requireSession(id); touch(session);
  const x = clamp(xInput, 0, session.width - 1); const y = clamp(yInput, 0, session.height - 1);
  const element = await elementAt(session.page, x, y).catch(() => null);
  await session.page.mouse.move(x, y, { steps: 8 }); await delay(45);
  ...
  await settle(session.page, 220); await handleAdRedirect(session);
...
export async function typeLiveBrowser(id: string, text: string, replace = true) {
  ...
  if (replace) {
    await session.page.keyboard.down("Control"); await session.page.keyboard.press("A");
...
export async function keyLiveBrowser(id: string, keyInput: string) {
  const session = requireSession(id); touch(session); const key = normalizeKey(keyInput);
  ...
  await settle(session.page, 180); await handleAdRedirect(session); return publicState(session);
}

export async function scrollLiveBrowser(id: string, deltaYInput: number) {
  const session = requireSession(id); touch(session); const deltaY = clamp(deltaYInput, -4000, 4000);
  ...
  if (session.recording && Math.abs(deltaY) > 25) {
    ...
    if (previous?.actionType === "scroll" && Date.now() - Date.parse(previous.createdAt) < 1200) {
      previous.y = Number(previous.y ?? 0) + deltaY; previous.createdAt = new Date().toISOString();
...
export async function navigateLiveBrowser(id: string, value: string) {
  const session = requireSession(id); touch(session); const url = validateUrl(value);
  await session.page.goto(url, { waitUntil: "domcontentloaded", timeout: 60_000 }); await settle(session.page, 500);
  if (session.recording) addStep(session, { actionType: "goto", value: url, waitMs: 600, label: `Przejdź do ${url}` });
  rememberSafe(session); return publicState(session);
}
...
export async function commandLiveBrowser(id: string, command: "back" | "forward" | "reload") {
  ...
  await settle(session.page, 350); await handleAdRedirect(session); return publicState(session);
}

export function setLiveBrowserRecording(id: string, enabled: boolean) {
  const session = requireSession(id); session.recording = enabled; touch(session);
  addEvent(session, "info", enabled ? "Włączono nagrywanie." : "Wstrzymano nagrywanie."); return publicState(session);
}
export function setLiveBrowserAds(id: string, enabled: boolean) {
  const session = requireSession(id); session.autoAds = enabled; touch(session);
  addEvent(session, "ad", enabled ? "Włączono ochronę przed reklamami." : "Wyłączono ochronę przed reklamami."); return publicState(session);
}
export async function markLiveBrowserAd(id: string) {
  const session = requireSession(id); const host = safeHost(session.page.url()); if (host) session.adHosts.add(host);
  addEvent(session, "ad", `Oznaczono jako reklamę: ${host || session.page.url()}.`, session.page.url()); await returnSafe(session); return publicState(session);
}
export async function markLiveSearchField(id: string) {
  const session = requireSession(id); const active = await activeElement(session.page);
  if (!active?.editable || !active.selector) throw new Error("Kliknij najpierw pole wyszukiwania.");
  await session.page.evaluate((selector) => { const element = document.querySelector(selector) as HTMLElement | null; if (element) { element.style.outline = "3px solid #7c5cff"; element.style.outlineOffset = "2px"; } }, active.selector);
  addEvent(session, "info", `Pole wyszukiwania: ${active.selector}`); return publicState(session);
}
export function undoLiveBrowserStep(id: string) {
  const session = requireSession(id); const step = session.steps.pop();
  addEvent(session, "info", step ? `Usunięto krok: ${step.label}.` : "Brak kroków do usunięcia."); return publicState(session);
}
export function clearLiveBrowserSteps(id: string) { const session = requireSession(id); session.steps = []; addEvent(session, "info", "Wyczyszczono kroki."); return publicState(session); }
export async function closeLiveBrowser(id: string) { const session = sessions.get(id); if (!session || session.closing) return; session.closing = true; sessions.delete(id); await session.browser.close().catch(() => undefined); }
export async function closeAllLiveBrowsers() { await Promise.all([...sessions.keys()].map(closeLiveBrowser)); }
...
function attachListeners(session: Session) {
  session.page.on("framenavigated", (frame) => {
    ...
    addEvent(session, "navigation", `Nawigacja: ${short(frame.url())}`, frame.url()); void handleAdRedirect(session);
  });
...
async function handlePopup(session: Session, target: Target) {
  ...
  const popup = await target.page().catch(() => null); if (!popup) return; await delay(300);
  const popupHost = safeHost(popup.url()); const mainHost = safeHost(session.page.url());
  ...
  if (session.autoAds && likelyAd) {
    ...
    await popup.close().catch(() => undefined); await session.page.bringToFront().catch(() => undefined);
...
async function handleAdRedirect(session: Session) {
  ...
  addEvent(session, "ad", `Powrót z domeny reklamowej: ${host}.`, session.page.url()); await returnSafe(session);
}
async function returnSafe(session: Session) {
  ...
  await session.page.bringToFront().catch(() => undefined); rememberSafe(session);
}
function rememberSafe(session: Session) {
  const url = session.page.url(); const host = safeHost(url);
  if (!host || session.adHosts.has(host) || session.safeUrls.at(-1) === url) return;
  session.safeUrls.push(url); if (session.safeUrls.length > 25) session.safeUrls.shift();
}
...
function requireSession(id: string) { const session = sessions.get(id); if (!session || session.closing) throw new Error("Sesja Chromium wygasła albo nie istnieje."); return session; }
function ensureCleanup() {
  if (cleanupTimer) return;
  cleanupTimer = setInterval(() => { const cutoff = Date.now() - 30 * 60_000; for (const [id, session] of sessions) if (Date.parse(session.touchedAt) < cutoff) void closeLiveBrowser(id); }, 60_000);
  cleanupTimer.unref();
}
