...
app.get<{ Params: { type: "movie" | "series"; id: string } }>("/subtitles/:type/:id.json", async (request, reply) => { const params = z.object({ type: z.enum(["movie", "series"]), id: z.string().min(1) }).safeParse(request.params); if (!params.success) { reply.code(400); return { subtitles: [] }; } const requestBaseUrl = getEffectivePublicBaseUrl() ?? `${request.protocol}://${request.hostname}`; return toStremioSubtitleResponse(getSubtitleCache(params.data.type, params.data.id), requestBaseUrl); });
