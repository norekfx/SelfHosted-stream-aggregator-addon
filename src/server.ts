...
await app.register(async (adminApp) => {
  ...
  adminApp.get<{ Params: { id: string } }>("/admin/docchi/series/:id/status", async (request, reply) => {
    ...
    if (!params.success) { reply.code(400); return { error: "Invalid series id." }; }
...
app.get<{ Params: { type: "movie" | "series"; id: string } }>("/meta/:type/:id.json", async (request, reply) => {
  ...
  if (!shouldBypassMetadataCache()) { const cached = getCachedMeta(params.data.type, params.data.id); if (cached) return { meta: cached }; }
