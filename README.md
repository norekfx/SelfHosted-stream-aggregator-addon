...
Important playback rule: `Original` is the original selected file. All quality options such as `Auto`, `4K`, `1080p` and `720p` are intended to be server-side transcodes generated from that same selected original, not separate streams from different addons.

The stream endpoint now runs aggregation, validation and ranking. It exposes options only when one working original has been selected. Transcoding endpoints intentionally return `501` until FFmpeg/HLS is implemented.
